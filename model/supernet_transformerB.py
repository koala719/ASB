...
class Vision_TransformerSuper(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12, broad=1,
        ...
        self.sample_scale = (216 // 4) ** -0.5
        self.sample_broad = None
        ...
        self.avg_pool_h = nn.AdaptiveAvgPool2d((197, 4))
    ...
    def set_sample_config(self, config: dict):
        ...
        self.sample_broad = config['broad']
    ...
    def forward_features(self, x):
        ...
        # self.sample_broad = nn.parameter(torch.zeros(self.sample_layer_num))
        ...
        if idx < 12 and self.sample_broad[idx] == 1:
            Q_list.append(q)
            K_list.append(k)
            V_list.append(v)
        # elif idx > 12:
        if Q_list != []:
            ...
            x_ass = (Attn @ V).transpose(1,2).reshape(B, N, -1)
            x_ass = self.avg_pool(x_ass)
            # out = out.permute(2, 1, 0)
            # print(out.size(), '=======================')
            # print(x.shape)
            if x.shape[-1] != 192:
                x_ass = F.interpolate(x_ass, size=[x.shape[-1]], mode='linear', align_corners=False) 


            x = torch.add(x, x_ass*0.4)
